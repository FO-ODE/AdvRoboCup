...
#     def calculate_approach_pose(self, object_position_map):
#         """根据物体位置计算接近点（自动朝向物体）"""
#         obj_x = object_position_map.x
#         obj_y = object_position_map.y

#         # 计算朝向物体的 yaw 角
#         yaw = math.atan2(obj_y, obj_x)
#         rospy.loginfo(f"Approach yaw angle: {math.degrees(yaw):.1f} degrees")

#         # 根据 yaw 角反向偏移，得到机器人应站位置
#         offset_x = self.approach_distance * math.cos(yaw)
#         offset_y = self.approach_distance * math.sin(yaw)
#         robot_x = obj_x - offset_x
#         robot_y = obj_y - offset_y

#         # 生成朝向目标的四元数
#         q = quaternion_from_euler(0, 0, yaw)
#         self.fixed_orientation = Quaternion(x=q[0], y=q[1], z=q[2], w=q[3])
